	...
	public class OpenGLWinFormsControl:Control {
		...
		Gdi.PIXELFORMATDESCRIPTOR ChoosePixelFormatEx(System.IntPtr hdc,int p_bpp,int p_depth,int p_dbl,int p_acc,ref int formatRef){
			...
			if(num>0){
				...
				Logger.openlog.curSev=Logger.Severity.FATAL_ERROR;
				Logger.openlog.Log(this,"ChoosePixelFormatEx","Failed to choose a device context because there are none available!");
			}
		...
		public void TaoInitializeContexts() {
			if(!ContextsReady) {
				try{
					...
					Logger.openlog.Log(this,"TaoInitializeContexts",e.ToString());
				}
